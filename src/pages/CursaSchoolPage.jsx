...
const CursaSchoolPage = () => {
  ...
  const scrollToForm = () => {
    ...
    if (formElement) {
      ...
      // Tracking Meta Pixel - InitiateCheckout
      if (typeof window !== 'undefined' && window.fbq) {
        window.fbq('track', 'InitiateCheckout');
      }
      
      // Tracking existant (Google Analytics)
      trackCTA({
        name: 'scroll_to_form',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // Tracking Meta Pixel - Lead conversion
        if (typeof window !== 'undefined' && window.fbq) {
          const price = formData.offre === '3' ? 420 : formData.offre === '2' ? 240 : 100;
          window.fbq('track', 'Lead', {
            content_name: 'Formation Anglais Professionnel & Confiance à l\'Oral',
  ...
  // Meta Pixel - Uniquement pour cette page
  useEffect(() => {
    ...
    // ID du Meta Pixel pour cette page spécifique
    const META_PIXEL_ID = '1544182883676544';
    const SCRIPT_ID = 'meta-pixel-formation-anglais-script';
    const NOSCRIPT_ID = 'meta-pixel-formation-anglais-noscript';

    // Vérifier si le pixel est déjà chargé pour cette page
    if (document.getElementById(SCRIPT_ID)) {
      // Le script est déjà chargé, s'assurer que PageView est tracké
      if (window.fbq) {
    ...
    try {
      // Code Meta Pixel exact fourni par Meta Business Manager
      // IIFE (Immediately Invoked Function Expression) - Code Meta standard
      if (!window.fbq) {
        ...
        // Charger le script avec un ID unique pour cette page
        ...
        if (existingScript && !existingScript.id) {
          ...
          // Le script a déjà été injecté par l'IIFE ci-dessus
          // On marque le dernier script injecté avec notre ID
          const scripts = document.getElementsByTagName('script');
          const lastScript = scripts[scripts.length - 1];
          if (lastScript && lastScript.src === 'https://connect.facebook.net/en_US/fbevents.js') {
            lastScript.id = SCRIPT_ID;
          }
        }
      }

      // Initialiser le pixel avec l'ID fourni et tracker PageView
      // Le système de queue de Meta Pixel garantit que ces appels seront exécutés
      // même si le script n'est pas encore complètement chargé
      window.fbq('init', META_PIXEL_ID);
      window.fbq('track', 'PageView');

      // Ajouter le noscript pour le fallback (dans le body)
    ...
    return () => {
      // On ne supprime pas le script car Meta Pixel doit persister
      // mais on pourrait ajouter une logique de nettoyage si nécessaire
    };
